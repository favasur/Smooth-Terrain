...
public abstract class PlatformSpecificRenderChunkRebuildTaskMixin extends RenderChunkCompileTaskMixin implements ISmoothTerrainChunkSectionRenderBuilder {
	...
	public Iterator<BlockPos> noCubes$renderChunk(
		...
		VanillaRenderer.renderChunk(
			...
			(state, worldPos, modelData, layer, buffer, optiFineRenderEnv) -> dispatcher.renderBatched(
				...
				false, random, (ModelData) modelData, layer
			)
		);
		return iterator;
	}
